...
class YoutubeIE(YoutubeBaseInfoExtractor):
    ...
    _TESTS = [
        {
            ...
            'info_dict': {
                'id': 'BaW_jenozKc',
                'ext': 'mp4',
                'title': 'youtube-dl test video "\'/\\├ñΓå¡≡¥òÉ',
        ...
        # Non-square pixels
        {
            ...
            'info_dict': {
                ...
                'description': 'made by Wacom from Korea | σ¡ùσ╣ò&σèáµ▓╣µ╖╗Θåï by TY\'s Allen | µäƒΦ¼¥heylisa00cavey1001σÉîσ¡╕τå▒µâàµÅÉΣ╛¢µóùσÅèτ┐╗Φ¡»',
                'uploader': 'σ¡½ßäïßäà',
                'license': 'Standard YouTube License',
                'title': '[A-made] Φ«èµàïσªìσ¡ùσ╣òτëê σñ¬σªì µêæσ░▒µÿ»ΘÇÖµ¿úτÜäΣ║║',
            },
        },
        # url_encoded_fmt_stream_map is empty string
        {
            ...
            'info_dict': {
                'id': 'qEJwOuvDf7I',
                'ext': 'webm',
                'title': '╨₧╨▒╤ü╤â╨╢╨┤╨╡╨╜╨╕╨╡ ╤ü╤â╨┤╨╡╨▒╨╜╨╛╨╣ ╨┐╤Ç╨░╨║╤é╨╕╨║╨╕ ╨┐╨╛ ╨▓╤ï╨▒╨╛╤Ç╨░╨╝ 14 ╤ü╨╡╨╜╤é╤Å╨▒╤Ç╤Å 2014 ╨│╨╛╨┤╨░ ╨▓ ╨í╨░╨╜╨║╤é-╨ƒ╨╡╤é╨╡╤Ç╨▒╤â╤Ç╨│╨╡',
                'description': '',
                'upload_date': '20150404',
                'uploader_id': 'spbelect',
                'uploader': '╨¥╨░╨▒╨╗╤Ä╨┤╨░╤é╨╡╨╗╨╕ ╨ƒ╨╡╤é╨╡╤Ç╨▒╤â╤Ç╨│╨░',
            },
            'params': {
                'skip_download': 'requires avconv',
            },
    ...
    def _real_extract(self, url):
        ...
        m_episode = re.search(
            r'<div[^>]+id="watch7-headline"[^>]*>\s*<span[^>]*>.*?>(?P<series>[^<]+)</a></b>\s*S(?P<season>\d+)\s*ΓÇó\s*E(?P<episode>\d+)</span>',
            video_webpage)
        if m_episode:
...
class YoutubePlaylistIE(YoutubePlaylistBaseInfoExtractor):
    ...
    _TESTS = [{
        ...
        'info_dict': {
            'title': '2017 ΦÅ»Φ¬₧µ£Çµû░σû«µ¢▓ (2/24µ¢┤µû░)',
